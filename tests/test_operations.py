...
class TestCompensationStability:
    ...
    def test_underflow_handling(self):
        ...
        result, compensation = Operations.compensated_divide(
            tiny, AbsoluteValue(magnitude=1e50, direction=1.0)
        )
...
class TestACTCompliance:
    ...
    def test_stability_axiom_operations(self):
        ...
        result = av
        for _ in range(100):
            result, _ = Operations.compensated_multiply(
                result, AbsoluteValue(magnitude=1.0, direction=1.0)
            )
    ...
    def test_eternity_axiom_operations(self):
        ...
        assert (
            abs(original_ratio.numerical_value() - scaled_ratio.numerical_value())
            < ACT_EPSILON
        )
...
class TestMissingOperationsCoverage:
    ...
    def test_compensated_sqrt_negative_direction(self):
        ...
        with pytest.raises(
            ValueError, match="Cannot take square root of negative AbsoluteValue"
        ):
    ...
    def test_compensated_log_negative_direction(self):
        ...
        with pytest.raises(
            ValueError, match="Cannot take logarithm of negative AbsoluteValue"
        ):
    ...
    def test_compensated_exp_overflow_protection(self):
        ...
        large_value = AbsoluteValue(
            magnitude=200.0, direction=1.0
        )  # Use smaller value to trigger overflow protection
        ...
        assert (
            compensation < -100.0
        )  # Should have significant negative logarithmic compensation
    ...
    def test_interpolate_invalid_parameter(self):
        ...
        # Test t < 0
        with pytest.raises(
            ValueError, match=r"Interpolation parameter t must be in \[0, 1\] range"
        ):
    ...
    def test_sequence_sum_kahan_compensation(self):
        ...
        values = [
            AbsoluteValue(magnitude=1e16, direction=1),
            AbsoluteValue(magnitude=1.0, direction=1),
            AbsoluteValue(magnitude=1.0, direction=1),
        ]
        result, compensation = Operations.sequence_sum(values, use_compensation=True)
    ...
    def test_sequence_product_overflow_underflow(self):
        ...
        large_values = [
            AbsoluteValue(magnitude=1e50, direction=1),
            AbsoluteValue(magnitude=1e60, direction=1),
        ]
        result, compensation = Operations.sequence_product(
            large_values, use_compensation=True
        )
        ...
        result2, compensation2 = Operations.sequence_product(
            small_values, use_compensation=True
        )
    ...
    def test_compensated_divide_absolute_denominator(self):
        ...
        with pytest.raises(
            ValueError, match=r"Cannot divide by Absolute \(denominator magnitude=0\)"
        ):

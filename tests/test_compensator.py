...
from balansis import ACT_COMPENSATION_FACTOR, ACT_EPSILON
from balansis.core.absolute import AbsoluteValue
from balansis.core.eternity import EternalRatio, SingularPolicy
...
class TestCompensationStrategy:
    ...
    def test_custom_strategy(self):
        ...
        strategy = CompensationStrategy(
            stability_threshold=1e-10, balance_factor=0.8, max_iterations=50
        )
...
class TestCompensator:
    ...
    def setup_method(self):
        ...
        self.custom_compensator = Compensator(
            CompensationStrategy(stability_threshold=1e-10, compensation_factor=0.1)
        )
    ...
    def test_analyze_stability(self):
        ...
        unstable_values = [
            AbsoluteValue(magnitude=1e-16, direction=1),
            AbsoluteValue(magnitude=1e-16, direction=-1),
        ]
    ...
    def test_detect_compensation_need(self):
        ...
        compensations = self.compensator.detect_compensation_need(
            "multiply", [large_value]
        )
        ...
        compensations = self.compensator.detect_compensation_need(
            "divide", [AbsoluteValue(magnitude=1.0, direction=1), absolute_value]
        )
    ...
    def test_compensate_sequence(self):
        ...
        result = self.compensator.compensate_sequence(
            Operations.compensated_add, values
        )
        ...
        result = self.compensator.compensate_sequence(
            Operations.compensated_multiply, values
        )
        assert result.magnitude == 6.0
        assert result.direction == 1

        # Test empty sequence
        empty_result = self.compensator.compensate_sequence(
            Operations.compensated_add, []
        )
        assert empty_result.is_absolute()

        # Test single value
        single_result = self.compensator.compensate_sequence(
            Operations.compensated_add, [values[0]]
        )
    ...
    def test_set_strategy(self):
        ...
        new_strategy = CompensationStrategy(
            stability_threshold=1e-8, balance_factor=0.3
        )
...
class TestCompensationEdgeCases:
    ...
    def test_compensation_record_validation_edge_cases(self):
        ...
        with pytest.raises(
            ValueError, match="Compensation factor must be non-negative"
        ):
            CompensationRecord(
                operation_type="test",
                compensation_type=CompensationType.STABILITY,
        ...
        with pytest.raises(
            ValueError, match="Stability metric must be between 0 and 1"
        ):
            CompensationRecord(
                operation_type="test",
                compensation_type=CompensationType.STABILITY,
    ...
    def test_apply_stability_compensation_small_values(self):
        ...
        small_value = AbsoluteValue(
            magnitude=1e-16, direction=1
    ...
    def test_apply_overflow_compensation_large_values(self):
        ...
        large_value = AbsoluteValue(
            magnitude=1e200, direction=1
    ...
    def test_compensate_division_singularity_compensation(self):
        ...
        )  # Absolute denominator triggers singularity
    ...
    def test_compensate_sequence_generic_operation(self):
        ...
        values = [
            AbsoluteValue(
                magnitude=1e-16, direction=1
            ),  # Small value to trigger compensation
    ...
    def test_compensation_strategy_config_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Balance factor must be between 0.0 and 1.0"
        ):
            CompensationStrategy(balance_factor=1.5)  # Invalid > 1.0
    ...
    def test_analyze_stability_all_absolute_values(self):
        ...
        absolute_values = [AbsoluteValue.absolute(), AbsoluteValue.absolute()]
    ...
    def test_detect_compensation_underflow_case(self):
        ...
        small_value = AbsoluteValue(
            magnitude=1e-101, direction=1
        )  # Below 1e-100 threshold
        normal_value = AbsoluteValue(magnitude=1.0, direction=1)

        compensations = compensator.detect_compensation_need(
            "add", [small_value, normal_value]
        )
        assert CompensationType.UNDERFLOW in compensations
    ...
    def test_apply_stability_compensation_edge_case(self):
        ...
        very_small = AbsoluteValue(
            magnitude=1e-13, direction=1
        )  # Below 1e-12 threshold
    ...
    def test_compensate_power_overflow_handling(self):
        ...
        large_base = AbsoluteValue(
            magnitude=1e101, direction=1
        )  # Above 1e100 threshold
    ...
    def test_compensate_sequence_generic_operation(self):
        ...
        def generic_op(a, b):
            return (
                AbsoluteValue(magnitude=a.magnitude + b.magnitude, direction=1),
                1.0,
            )
    ...
    def test_compensate_multiplication_overflow_compensation_recording(self):
        ...
        large_a = AbsoluteValue(
            magnitude=1e101, direction=1
        )  # Above overflow threshold
    ...
    def test_compensate_division_non_absolute_denominator_singularity(self):
        ...
        compensations = compensator.detect_compensation_need(
            "invert", [numerator, small_denominator]
        )
    ...
    def test_compensate_sequence_with_stability_compensation(self):
        ...
        result = compensator.compensate_sequence(
            generic_op_with_compensation, unstable_values
        )
        assert isinstance(result, AbsoluteValue)
    ...
    def test_compensate_power_overflow_detection(self):
        ...
        large_base = AbsoluteValue(
            magnitude=1e150, direction=1
        )  # Exceeds default overflow threshold
        normal_exponent = AbsoluteValue(magnitude=2.0, direction=1)

        # This should detect overflow compensation need
        compensations = compensator.detect_compensation_need(
            "power", [large_base, normal_exponent]
        )
        assert CompensationType.OVERFLOW in compensations

        # Test that overflow compensation is applied
        compensated_values = compensator.apply_overflow_compensation(
            [large_base, normal_exponent]
        )
        assert (
            compensated_values[0].magnitude <= compensator.strategy.overflow_threshold
        )
    ...
    def test_compensate_underflow_detection_multiplication(self):
        ...
        small_a = AbsoluteValue(
            magnitude=1e-150, direction=1
        )  # Below default underflow threshold
        small_b = AbsoluteValue(magnitude=1e-150, direction=-1)

        # This should detect underflow compensation need
        compensations = compensator.detect_compensation_need(
            "multiply", [small_a, small_b]
        )
        assert CompensationType.UNDERFLOW in compensations

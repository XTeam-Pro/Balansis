...
from balansis import AbsoluteValue, Compensator, Operations
...
class AccuracyBenchmark:
    ...
    def decimal_sum(self, data: List[float]) -> float:
        with localcontext() as context:
            context.prec = 50
            return float(sum((Decimal.from_float(float(x)) for x in data), Decimal(0)))
    ...
    def act_sum(self, data: List[float]) -> float:
        ...
        result, _ = self.ops.sequence_sum(absolute_data)
        return result.to_float()

    def calculate_reference_value(self, data: List[float]) -> float:
        return float(sum((Fraction(float(x)) for x in data), Fraction()))
    ...
    def run_accuracy_test(self, size: int, scenario: str) -> Dict[str, Any]:
        ...
        for method_name, method_func in methods.items():
            try:
                ...
                relative_error = (
                    absolute_error / abs(reference) if reference != 0 else float("inf")
                )
                ...
                results["methods"][method_name] = {
                    ...
                    "log10_abs_error": (
                        math.log10(absolute_error)
                        if absolute_error > 0
                        else -float("inf")
                    ),
                    "log10_rel_error": (
                        math.log10(relative_error)
                        if relative_error > 0
                        else -float("inf")
                    ),
                }
    ...
    def analyze_stability_patterns(self, data: List[float]) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "sign_changes": sum(
                1 for i in range(1, len(data)) if data[i] * data[i - 1] < 0
            ),
        ...
        for i in range(len(absolute_data)):
            for j in range(i + 1, len(absolute_data)):
                if (
                    absolute_data[i].direction != absolute_data[j].direction
                    and abs(absolute_data[i].magnitude - absolute_data[j].magnitude)
                    < 1e-10
                ):
    ...
    def generate_accuracy_report(self, results: Dict[str, List[Dict[str, Any]]]) -> str:
        ...
        report.append(
            "| Сценарий | Размер | Метод | Абсолютная ошибка | Относительная ошибка | Время (мс) |"
        )
        report.append(
            "|----------|--------|-------|-------------------|---------------------|------------|"
        )
            ...
            for test_result in scenario_results:
                ...
                for method_name, method_result in test_result["methods"].items():
                    if "error" not in method_result:
                        ...
                        report.append(
                            f"| {scenario} | {size} | {method_name} | {abs_err} | {rel_err} | {time_ms} |"
                        )
        ...
        for scenario, scenario_results in results.items():
            ...
            for test_result in scenario_results:
                ...
                best_method = min(
                    ...
                    key=lambda x: (
                        x[1]["absolute_error"] if "error" not in x[1] else float("inf")
                    ),
                )

                report.append(
                    f"**Размер {size}:** Лучший метод - {best_method[0]} "
                    f"(абс. ошибка: {best_method[1]['absolute_error']:.2e})"
                )

                # Сравнение ACT с другими методами
                if (
                    "act" in test_result["methods"]
                    and "error" not in test_result["methods"]["act"]
                ):
                    ...
                    if act_error < float64_error:
                        improvement = (
                            float64_error / act_error if act_error > 0 else float("inf")
                        )
                        report.append(
                            f"  - ACT улучшение над float64: {improvement:.1f}x"
                        )
                    else:
                        degradation = (
                            act_error / float64_error
                            if float64_error > 0
                            else float("inf")
                        )
        ...
        report.append(f"\n**Рекомендации:**")
        report.append(
            "- ACT показывает наилучшие результаты в сценариях с катастрофическим сокращением"
        )
        report.append(
            "- Для общих вычислений Kahan summation остается конкурентоспособным"
        )
        report.append(
            "- Decimal обеспечивает высокую точность, но за счет производительности"
        )
        report.append(
            "- ACT особенно эффективен при работе с данными разных порядков величин"
        )

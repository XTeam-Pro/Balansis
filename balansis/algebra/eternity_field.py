...
from .group import AbsoluteGroup, GroupElement, GroupOperation
...
class FieldElement(BaseModel):
    ...
    ratio: EternalRatio
    additive_order: Optional[int] = Field(
        default=None, description="Order in additive group"
    )
    multiplicative_order: Optional[int] = Field(
        default=None, description="Order in multiplicative group"
    )
    minimal_polynomial: Optional[List[float]] = Field(
        default=None, description="Minimal polynomial coefficients"
    )
    ...
    def is_zero(self) -> bool:
        ...
        """
        return (
            self.ratio.numerator.is_absolute()
            and not self.ratio.denominator.is_absolute()
        )
...
class EternalRatioOperation(FieldOperation):
    ...
    def add(self, a: FieldElement, b: FieldElement) -> FieldElement:
        ...
        # (a/b) + (c/d) = (ad + bc)/(bd)
        ad = self.compensator.compensate_multiplication(
            ratio_a.numerator, ratio_b.denominator
        )
        bc = self.compensator.compensate_multiplication(
            ratio_b.numerator, ratio_a.denominator
        )
        bd = self.compensator.compensate_multiplication(
            ratio_a.denominator, ratio_b.denominator
        )
    ...
    def multiply(self, a: FieldElement, b: FieldElement) -> FieldElement:
        ...
        # (a/b) * (c/d) = (ac)/(bd)
        ac = self.compensator.compensate_multiplication(
            ratio_a.numerator, ratio_b.numerator
        )
        bd = self.compensator.compensate_multiplication(
            ratio_a.denominator, ratio_b.denominator
        )
    ...
    def additive_inverse(self, element: FieldElement) -> FieldElement:
        ...
        inverse_ratio = EternalRatio(
            numerator=negated_numerator, denominator=ratio.denominator
        )
...
class PrimeFieldOperation(EternalRatioOperation):
    """Exact arithmetic on residues in a prime field."""
    ...
    def residue(self, element: FieldElement) -> int:
        ratio = element.ratio
        value = Fraction(ratio.numerator.to_float()) / Fraction(
            ratio.denominator.to_float()
        )
        return (value.numerator * pow(value.denominator, -1, self.prime)) % self.prime

    def element(self, value: int) -> FieldElement:
        return FieldElement(ratio=EternalRatio.from_float(float(value % self.prime)))

    def add(self, a: FieldElement, b: FieldElement) -> FieldElement:
        return self.element(self.residue(a) + self.residue(b))

    def multiply(self, a: FieldElement, b: FieldElement) -> FieldElement:
        return self.element(self.residue(a) * self.residue(b))

    def additive_inverse(self, element: FieldElement) -> FieldElement:
        return self.element(-self.residue(element))

    def multiplicative_inverse(self, element: FieldElement) -> FieldElement:
        value = self.residue(element)
        if value == 0:
            raise ValueError("Cannot invert zero element")
        return self.element(pow(value, -1, self.prime))
...
class FieldGroupOperation(GroupOperation):
    ...
    def apply(self, a: GroupElement, b: GroupElement) -> GroupElement:
        left = FieldElement(ratio=EternalRatio.from_float(a.value.to_float()))
        right = FieldElement(ratio=EternalRatio.from_float(b.value.to_float()))
        operation = self.field.multiply if self.multiplicative else self.field.add
        return GroupElement(value=operation(left, right).ratio.to_absolute_value())

    def identity(self) -> GroupElement:
        return GroupElement(
            value=AbsoluteValue.from_float(1.0 if self.multiplicative else 0.0)
        )

    def inverse(self, element: GroupElement) -> GroupElement:
        value = FieldElement(ratio=EternalRatio.from_float(element.value.to_float()))
        operation = (
            self.field.multiplicative_inverse
            if self.multiplicative
            else self.field.additive_inverse
        )
        return GroupElement(value=operation(value).ratio.to_absolute_value())
...
class EternityField:
    ...
    def __init__(
        self,
        operation: Optional[EternalRatioOperation] = None,
        ...
        self.finite = finite
        self._zero: Optional[FieldElement] = None
        self._one: Optional[FieldElement] = None
    ...
    @classmethod
    def finite_field(
        cls, prime: int, degree: int = 1, compensator: Optional[Compensator] = None
    ) -> "EternityField":
        """Construct a prime field with integer residue representatives."""
        if (
            not isinstance(prime, int)
            or isinstance(prime, bool)
            or not cls._is_prime(prime)
        ):
            raise ValueError(f"{prime} is not prime")
        if not isinstance(degree, int) or isinstance(degree, bool) or degree < 1:
            raise ValueError("Degree must be at least 1")
        if degree != 1:
            raise NotImplementedError("Extension fields require a polynomial basis")
        operation = PrimeFieldOperation(prime)
        field = cls(
            operation=operation,
            elements={operation.element(i) for i in range(prime)},
    ...
    def _reduce_modulo_characteristic(self, element: FieldElement) -> FieldElement:
        if self.characteristic == 0:
            return element
        operation = PrimeFieldOperation(self.characteristic)
        return operation.element(operation.residue(element))
    ...
    def additive_group(self) -> AbsoluteGroup:
        ...
        elements = {
            GroupElement(value=e.ratio.to_absolute_value()) for e in self.elements
        }
        return AbsoluteGroup(
            operation=FieldGroupOperation(self, False), elements=elements, finite=True
        )
    ...
    def multiplicative_group(self) -> AbsoluteGroup:
        ...
        elements = {
            GroupElement(value=e.ratio.to_absolute_value())
            for e in self.elements
            if not e.is_zero()
        }
        return AbsoluteGroup(
            operation=FieldGroupOperation(self, True), elements=elements, finite=True
        )
    ...
    def __repr__(self) -> str:
        ...
        if self.finite:
            return (
                f"EternityField(char={self.characteristic}, order={len(self.elements)})"
            )
...
class Polynomial:
    ...
    def __init__(self, coefficients: List[FieldElement], ring: PolynomialRing):
        ...
        Args:
            ...
            ring: Parent polynomial ring
        """
        self.coefficients = self._normalize_coefficients(list(coefficients))
        self.ring = ring
    ...
    def __add__(self, other: "Polynomial") -> "Polynomial":
        ...
        self_coeffs = self.coefficients + [self.ring.field.zero()] * (
            max_len - len(self.coefficients)
        )
        other_coeffs = other.coefficients + [self.ring.field.zero()] * (
            max_len - len(other.coefficients)
        )
    ...
    def __mul__(self, other: "Polynomial") -> "Polynomial":
        ...
        for i, a_coeff in enumerate(self.coefficients):
            for j, b_coeff in enumerate(other.coefficients):
                ...
                result_coeffs[i + j] = self.ring.field.add(
    ...
    def evaluate(self, value: FieldElement) -> FieldElement:
        ...
        for i in range(len(self.coefficients) - 2, -1, -1):
            result = self.ring.field.add(
                self.ring.field.multiply(result, value), self.coefficients[i]
            )

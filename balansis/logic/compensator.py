...
class CompensationRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "compensated_values": [
                self._serialize_value(v) for v in self.compensated_values
            ],
...
class CompensationStrategy(BaseModel):
    ...
    max_iterations: int = Field(
        default=100, description="Maximum iterations for convergence compensation"
    )
    convergence_tolerance: float = Field(
        default=1e-10, description="Tolerance for convergence checks"
    )
    balance_factor: float = Field(
        default=0.5, description="Factor for balance compensation (0.0 to 1.0)"
    )
...
class Compensator:
    ...
    def analyze_stability(self, values: List[AbsoluteValue]) -> float:
        ...
        magnitudes = [m / scale for m in magnitudes]
    ...
    def detect_compensation_need(
        self, operation: str, operands: List[AbsoluteValue]
        ...
        if operation in ["divide", "invert"] and any(
            op.is_absolute() for op in operands[1:]
        ):
        ...
        if operation in ["add", "subtract"] and len(operands) == 2:
            ...
            if (
                not a.is_absolute()
                and not b.is_absolute()
                and abs(a.magnitude - b.magnitude) < self.strategy.stability_threshold
    ...
    def apply_stability_compensation(
        self, values: List[AbsoluteValue]
    ) -> List[AbsoluteValue]:
    ...
    def apply_overflow_compensation(
        self, values: List[AbsoluteValue]
    ) -> List[AbsoluteValue]:
    ...
    def apply_balance_compensation(
        self, a: AbsoluteValue, b: AbsoluteValue
    ) -> Tuple[AbsoluteValue, AbsoluteValue]:
    ...
    def compensate_addition(self, a: AbsoluteValue, b: AbsoluteValue) -> AbsoluteValue:
        ...
        result, op_compensation = Operations.compensated_add(
            comp_a, comp_b, compensation_factor
        )
    ...
    def compensate_division(
        self, numerator: AbsoluteValue, denominator: AbsoluteValue
    ) -> EternalRatio:
        ...
        # Detect needed compensations
        compensations = self.detect_compensation_need(
        ...
        if CompensationType.SINGULARITY in compensations:
            if denominator.is_absolute():
                ...
                comp_denominator = AbsoluteValue(
                    magnitude=self.strategy.stability_threshold, direction=1
                )
            else:
        ...
        # Perform the operation
        result, op_compensation = Operations.compensated_divide(
            numerator, comp_denominator
        )
    ...
    def compensate_division_extended(
        ...
        result, op_compensation = Operations.compensated_divide_extended(
        ...
        *,
        ...
        if compensations or event is not None:
            record = CompensationRecord(
                ...
                compensation_type=(
                    compensations[0] if compensations else CompensationType.SINGULARITY
                ),
                original_values=[numerator, denominator],
    ...
    def compensate_sequence(
        self, operation: Callable, values: List[AbsoluteValue]
    ) -> AbsoluteValue:
    ...
    def get_compensation_summary(self) -> Dict[str, Any]:
        ...
        compensation_types: dict[str, int] = {}
        total_stability = 0.0
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Compensator(operations={self._operation_count}, "
    ...
    def __str__(self) -> str:
        ...
        return (
            f"Compensator: {summary['compensated_operations']}/{summary['total_operations']} "
    ...
    def compensate_array(
        self, arr_a: List[AbsoluteValue], arr_b: List[AbsoluteValue]
    ) -> List[AbsoluteValue]:

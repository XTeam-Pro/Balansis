...
# NumPy is required; plotting engines are loaded only by this optional module.
import importlib
import math
...
plt: Any = _optional_module("matplotlib.pyplot")
patches: Any = _optional_module("matplotlib.patches")
_colors = _optional_module("matplotlib.colors")
_animation = _optional_module("matplotlib.animation")
LinearSegmentedColormap: Any = getattr(_colors, "LinearSegmentedColormap", None)
FuncAnimation: Any = getattr(_animation, "FuncAnimation", None)
MATPLOTLIB_AVAILABLE = plt is not None

go: Any = _optional_module("plotly.graph_objects")
px: Any = _optional_module("plotly.express")
_subplots = _optional_module("plotly.subplots")
make_subplots: Any = getattr(_subplots, "make_subplots", None)
ff: Any = _optional_module("plotly.figure_factory")
PLOTLY_AVAILABLE = go is not None

from ..core.absolute import AbsoluteValue
from ..core.eternity import EternalRatio
from ..core.operations import Operations
from ..logic.compensator import CompensationRecord, Compensator


def _record_number(values: list[Any]) -> float:
    for value in values:
        if isinstance(value, AbsoluteValue):
            return value.to_float()
        if hasattr(value, "numerical_value"):
            number = value.numerical_value()
...
class PlotConfig(BaseModel):
    ...
    style: PlotStyle = Field(
        default=PlotStyle.SCIENTIFIC, description="Plot style theme"
    )
    backend: PlotBackend = Field(
        default=PlotBackend.MATPLOTLIB, description="Plotting backend"
    )
    ...
    animation_duration: int = Field(
        default=1000, description="Animation duration in milliseconds"
    )
...
class PlotUtils:
    ...
    def _validate_dependencies(self) -> None:
        ...
        if not NUMPY_AVAILABLE:
            raise ImportError(
                "NumPy is required for plotting functionality. Install with: pip install numpy"
            )
    ...
    def _plot_absolute_values_matplotlib(
        self,
        values: List[AbsoluteValue],
        title: str,
        xlabel: str,
        ...
        colors = [
            self.config.color_palette[0] if d > 0 else self.config.color_palette[1]
            for d in directions
        ]
        ...
        if self.config.legend:
            ...
            positive_patch = patches.Patch(
                color=self.config.color_palette[0], label="Positive Direction"
            )
            negative_patch = patches.Patch(
                color=self.config.color_palette[1], label="Negative Direction"
            )
    ...
    def _plot_absolute_values_plotly(
        self,
        values: List[AbsoluteValue],
        title: str,
        xlabel: str,
        ...
        absolute_indices = [i for i, d in enumerate(directions) if False]
    ...
    def _plot_eternal_ratios_matplotlib(
        self,
        ratios: List[EternalRatio],
        title: str,
        xlabel: str,
        ...
        fig, (ax1, ax2) = plt.subplots(
            2, 1, figsize=(self.config.width / 100, self.config.height / 100 * 1.2)
        )
        ...
        colors = [
            self.config.color_palette[0] if stable else self.config.color_palette[3]
        ...
        if show_stability and self.config.legend:
            stable_patch = patches.Patch(
                color=self.config.color_palette[0], label="Stable"
            )
            unstable_patch = patches.Patch(
                color=self.config.color_palette[3], label="Unstable"
            )
    ...
    def _plot_eternal_ratios_plotly(
        self,
        ratios: List[EternalRatio],
        title: str,
        xlabel: str,
        ...
        fig.update_xaxes(
            title_text=xlabel, title_font=dict(size=self.config.axis_label_size)
        )
        fig.update_yaxes(
            title_text="Numerical Value",
            title_font=dict(size=self.config.axis_label_size),
            row=1,
            col=1,
        )
        fig.update_yaxes(
            title_text="Signed Value",
            title_font=dict(size=self.config.axis_label_size),
            row=2,
            col=1,
        )
    ...
    def plot_compensation_analysis(
        self,
    ...
    def _plot_compensation_matplotlib(
        self, records: List[CompensationRecord], title: str, save_path: Optional[str]
    ) -> Any:
        """Plot compensation analysis using matplotlib."""
        fig, axes = plt.subplots(
            2,
            2,
            figsize=(self.config.width / 100 * 1.5, self.config.height / 100 * 1.2),
        )
        ax1, ax2 = axes[0]
        ax3, ax4 = axes[1]
        ...
        original_values = [_record_number(record.original_values) for record in records]
        compensated_values = [
            _record_number(record.compensated_values) for record in records
        ]
        ...
        ax1.bar(
            type_counts.keys(),
            type_counts.values(),
            color=self.config.color_palette[: len(type_counts)],
            alpha=self.config.alpha,
        )
        ax1.set_title(
            "Compensation Types Distribution", fontsize=self.config.title_size
        )
        ...
        effectiveness = [
            abs(comp - orig) / max(abs(orig), 1e-10)
        ...
        ax4.set_title(
            "Compensation Effectiveness Distribution", fontsize=self.config.title_size
        )
    ...
    def _plot_compensation_plotly(
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines+markers",
                line=dict(
                    color=self.config.color_palette[2], width=self.config.line_width
                ),
    ...
    def _plot_phase_space_matplotlib(
        self, values: List[AbsoluteValue], title: str, save_path: Optional[str]
    ) -> Any:
        ...
        ax.axhline(
            y=0, color="blue", linestyle="--", alpha=0.7, label="Direction Boundary"
        )
    ...
    def _plot_phase_space_plotly(
        ...
        fig.update_layout(
            title=dict(text=title, font=dict(size=self.config.title_size)),
            xaxis_title=dict(
                text="Magnitude", font=dict(size=self.config.axis_label_size)
            ),
            yaxis_title=dict(
                text="Direction", font=dict(size=self.config.axis_label_size)
            ),
            showlegend=self.config.legend,
            width=self.config.width,
            height=self.config.height,
                ...
                marker=dict(
                    color=[
                        (
                            ...
                            if d > 0
                            else self.config.color_palette[1]
                        )
                        for d in directions
                    ],
                    size=self.config.marker_size,
    ...
    def animate_sequence_evolution(
        self,
        sequences: List[List[AbsoluteValue]],
    ...
    def _animate_matplotlib(
        self, sequences: List[List[AbsoluteValue]], title: str, save_path: Optional[str]
    ) -> FuncAnimation:
        ...
        time_text = ax.text(
            0.02, 0.95, "", transform=ax.transAxes, fontsize=self.config.font_size
        )

        def animate(frame: int) -> tuple[Any, Any, Any]:
            """Animation function."""
        ...
        # Create animation
        anim = FuncAnimation(
            fig, animate, frames=len(sequences), interval=200, blit=True, repeat=True
        )
    ...
    def export_plot_data(
        ...
        """Export one typed record per value, ratio, or compensation event."""
        import json

        import pandas as pd

        records = []
        for value in absolute_values or []:
            records.append(value.to_json())
        for ratio in eternal_ratios or []:
            records.append(ratio.to_json())
        for record in compensation_records or []:
            records.append({"type": "CompensationRecord", **record.to_dict()})
        if not records:
